...
const App: React.FC = () => {
  ...
  const handleNavigate = (screen: Screen, recipeId?: string, shouldAutoCreate: boolean = false) => {
    // FIX: Reset ID if undefined, otherwise set it
  ...
  const renderScreen = () => {
    const ScreenComponent = () => {
        ...
        switch (currentScreen) {
            ...
            // FIX: Pass the activeRecipeId to Formulator
            case 'formulator': return <Formulator onBack={() => setCurrentScreen('dashboard')} autoCreate={autoCreate} userName={currentUserEmail} initialRecipeId={activeRecipeId} />;
    ...
    <div className="flex flex-col min-h-screen bg-background-light dark:bg-background-dark transition-colors duration-300">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-[#101622] border-t border-slate-200 dark:border-slate-800 pb-safe shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
        <div className="flex items-end justify-between px-2 pt-2 pb-1 max-w-lg mx-auto">
          ...
          {showFormulas && <NavBtn icon="science" label="Formula" active={currentScreen === 'formulator'} onClick={() => handleNavigate('formulator')} />}
          
          {showProducts && (
            <div className="relative -top-6 mx-1">
              <button onClick={() => handleNavigate('products')} className={`flex items-center justify-center size-14 rounded-full shadow-xl border-4 border-white dark:border-[#0b0f19] transition-transform active:scale-95 ${currentScreen === 'products' ? 'bg-primary text-white' : 'bg-slate-900 text-white'}`}>
          ...
          {showQuotes && <NavBtn icon="request_quote" label="Quotes" active={currentScreen === 'quote'} onClick={() => handleNavigate('quote')} />}
...
const NavBtn = ({ icon, label, active, onClick, isDanger }: any) => (
  <button onClick={onClick} className={`flex-1 min-w-0 flex flex-col items-center gap-1 py-1 group ${active ? 'text-primary' : isDanger ? 'text-slate-300 hover:text-red-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}>
    <span className={`material-symbols-outlined text-2xl ${active ? 'filled' : ''}`}>{icon}</span>
    <span className="text-[10px] font-bold uppercase tracking-tight w-full text-center whitespace-nowrap overflow-hidden text-ellipsis px-1">{label}</span>
  </button>
);
