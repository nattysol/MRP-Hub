...
interface DashboardProps {
  ...
  role: UserRole | null; // <--- Accept Role
}
  ...
  return (
    <div className="flex-1 flex flex-col pb-24 px-6 pt-10">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-black text-slate-800 dark:text-white mb-1"> Manufacturing Hub</h1>
        <div className="flex items-center gap-2">
           <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
             role === 'admin' ? 'bg-purple-100 text-purple-600' :
             role === 'production' ? 'bg-blue-100 text-blue-600' :
             'bg-emerald-100 text-emerald-600'
           }`}>
             {role === 'ops' ? 'Sales & Ops' : role} View
           </span>
           <p className="text-slate-500 font-medium">Dashboard</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        ...
        {canSeeProduction && (
          <DashboardCard 
            ...
            onClick={() => onNavigate('formulator', undefined, true)} 
          />
        )}
        ...
        {canSeeOps && (
          <DashboardCard 
            icon="request_quote" 
            label="Quote Gen" 
            color="bg-white dark:bg-card-dark text-slate-600 dark:text-slate-200 border border-slate-200 dark:border-slate-800" 
            onClick={() => onNavigate('quote', undefined, true)} 
          />
        )}
      ...
      <div className="flex gap-4 mb-8">
        {canSeeProduction && (
          <div className="flex-1 bg-white dark:bg-card-dark p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-800 dark:text-white">{stats.recipes}</p>
              <p className="text-[10px] uppercase font-bold text-slate-400">Formulas</p>
            </div>
          </div>
        )}
        {(canSeeProduction || canSeeOps) && (
          <div className="flex-1 bg-white dark:bg-card-dark p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-800 dark:text-white">{stats.ingredients}</p>
      ...
      {/* Recent Activity - Only for Production/Admin */}
      {canSeeProduction && (
        <>
          ...
          <div className="flex flex-col gap-3">
            {recentRecipes.map(r => (
              <button 
                key={r.id} 
                onClick={() => onNavigate('products', r.id)}
                className="flex items-center justify-between p-4 bg-white dark:bg-card-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm hover:border-primary/50 transition-colors text-left group"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-slate-800 dark:text-white text-sm group-hover:text-primary transition-colors">{r.name}</p>
                    <p className="text-xs text-slate-400">{r.date} • {r.project}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
